...
class ConvAutoencoder(BaseModule):
    ...
    def __init__(
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(
                in_channels=chs[0], out_channels=chs[1], kernel_size=self.k_size, stride=1, padding=1
            ),  # Output - (B, 32, W, H)
            nn.BatchNorm2d(
                chs[1]
            ),  # Normalise each output to smoothen the loss plot wrt parameters - loss will converge better #output shape unchanged as above
            nn.ReLU(inplace=True),  # Activation, shape unchanged
            nn.MaxPool2d(
                kernel_size=2, stride=2
            ),  # Reduces feature maps by taking the max value in each region, output shape is (B, 32, 32, 32)
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Upsample(scale_factor=2, mode="nearest"),  # (B, 256, 4, 4)
            nn.Conv2d(
                chs[1], self.in_channels, kernel_size=self.k_size, padding=1
            ),  # (B, 128, 4, 4) #No other layers as final layer; output not passed
        )
    ...
    def training_step_body(self, batch, batch_idx):
        ...
        # print(x.shape)
        reconstructed, z = self(
            x
        )  # self(x) is equivalent to self.forward(x), but we should call it as self(x) (that’s the PyTorch usual way).
...
class ConvAutoencoderWithLocation(ConvAutoencoder):
    ...
    def validation_step_body(self, batch, batch_idx):
        x, coords = (
            batch  # input and coords - there must be a better way to call batch elements, perhaps calling them by pre-specified keys
        )
        reconstructed, z = self(x)  # self(x) is equivalent to self.forward(x)
