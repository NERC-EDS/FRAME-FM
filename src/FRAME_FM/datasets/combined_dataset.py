...
class CorrespondingTilesDataset(Dataset):
    def __init__(
        self, datasets: list[BaseDataset], crs_specs: list[CRS_conversion_spec | tuple | list | None] | None = None
    ) -> None:
        if crs_specs is None:
            crs_specs = [None for _ in range(len(datasets))]
        if len(datasets) != len(crs_specs):
            raise ValueError(f"Length of datasets ({len(datasets)}) must equal that of crs_specs ({len(crs_specs)}).")
        self.datasets = datasets
        ...
        self.index_mappers = [
            TiledIndexMapper.from_tiled_array(dataset.data)
    ...
    def __getitem__(self, index) -> list:
        ref_location = {
            dim: self.datasets[0].data[index][dim].mean().values for dim in self.datasets[0].data[index].dims
        }
        ...
        for dataset, crs_convertor, index_mapper in ds_iterable:
            ...
            # (DataArray.dims may in theory be Hashable, but in practice are str)
            search_coords = {
                dim: coord for dim, coord in search_coords.items() if dim in index_mapper.tile_sizes.keys()
            }
