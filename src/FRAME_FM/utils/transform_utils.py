...
def _parse_CRS_spec(spec: CRS_spec | tuple[int, dict[str, str]] | list):
    if isinstance(spec, CRS_spec):
        ...
        if len(spec) != 2:
            raise ValueError(f"If a tuple or list, CRS spec {spec} must be of form (EPSG, dim_mapping_dict).")
...
class CRS_conversion_spec:
    ...
    def __init__(
        self, source: CRS_spec | tuple[int, dict[str, str]] | list, target: CRS_spec | tuple[int, dict[str, str]] | list
    ):
...
class CRS_convertor:
    ...
    def __init__(self, conversion_spec: CRS_conversion_spec | tuple | list):
        if isinstance(conversion_spec, tuple | list):
            ...
            raise TypeError(f"conversion_spec {conversion_spec} must be a list, tuple, or CRS_conversion_spec.")
        source_crs, self.source_axis_abbrvs = _implement_crs_spec(conversion_spec.source)
        self.source_dims = [conversion_spec.source.dim_mapping[abbrv] for abbrv in self.source_axis_abbrvs]
        ...
        self.axis_id_mapping = {
            dim: target_abbrvs.index(abbrv) for abbrv, dim in conversion_spec.target.dim_mapping.items()
        }
    ...
    def add_converted_coords(self, sample: DataArray, dims: set[str]) -> DataArray:
        ...
        if len(missing_dims) > 0:
            raise ValueError(f"Source CRS mapping dims {missing_dims} must be among sample.dims {sample.dims}")
        ...
        return sample.assign_coords({dim: (self.source_dims, coord_arrays[self.axis_id_mapping[dim]]) for dim in dims})
    ...
    def transform(self, location: dict[str, ndarray], inverse: bool = False) -> dict[str, ndarray]:
        ...
        target_coords = self.crs_transformer.transform(
            *[location[dim] for dim in source_dims],
