...
class PatchEmbed(BaseEmbedder):
    ...
    def __init__(
        ...
        **conv_kwargs,
        ...
        """
        super().__init__()
        assert len(input_shape) in _Conv_dim_dict.keys(), f"{len(input_shape)}D input not supported"
        ...
        self.proj = conv_class(
            n_channels, embed_dim, kernel_size=patch_shape, stride=patch_shape, bias=bias, **conv_kwargs
        )
        ...
        self.reconstruct_layer = torch.nn.Linear(reconstruct_dim, prod(patch_shape) * n_channels, bias=True)
    ...
    def _define_position_embedding(self, embed_dim: int) -> torch.nn.Parameter:
        ...
        grid = torch.meshgrid([torch.arange(grid_s, dtype=torch.float32) for grid_s in self.grid_shape], indexing="ij")
    ...
    def tokenify(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        assert len(inputs.shape) - 2 == n_dim, f"{len(inputs.shape) - 2}-D input not divisible into {n_dim}-D patches"
        for dim, (s_i, s_p) in enumerate(zip(inputs.shape[2:], patch_shape)):
            assert s_i % s_p == 0, f"Input dimension {dim} not divisible into patches ({s_i} % {s_p} != 0)"

        grid_shape, n_patches = self._count_patches(inputs.shape[2:])
        x = inputs.reshape(
            shape=(
                (inputs.shape[0], self.n_channels) + sum([(s_g, s_p) for s_g, s_p in zip(grid_shape, patch_shape)], ())
            )
        )
    ...
    def untokenify(self, x: torch.Tensor, output_shape: tuple[int, ...] | None = None) -> torch.Tensor:
        ...
        if output_shape is None:
            ...
            assert len(output_shape) == n_dim, f"{len(output_shape)}-D output not formable from {n_dim}-D patches"
            grid_shape, n_patches = self._count_patches(output_shape)
        assert x.shape[1] == n_patches, f"Grid shape {grid_shape} not formable from {x.shape[1]} patches"
        ...
        imgs = x.reshape(
            shape=((x.shape[0], n_channels) + sum([(s_g * s_p,) for s_g, s_p in zip(grid_shape, patch_shape)], ()))
        )
        # (N, C, H h, W w) for 2D patches
        return imgs
...
class STPatchEmbed(PatchEmbed):
    ...
    def _define_position_embedding(
        ...
        def embedding(pos: torch.Tensor) -> torch.Tensor:
            ...
            for dim, (s_pos, s_spec) in enumerate(zip(pos.shape[2:], self.input_shape)):
                assert s_pos == s_spec, f"Input positions dimension {dim} ({s_pos}) doesn't match spec. ({s_spec})"
            pos = conv_fn(
    ...
    def forward(self, st_input: tuple[torch.Tensor, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for dim, (s_x, s_spec) in enumerate(zip(x.shape[2:], self.input_shape)):
            assert s_x == s_spec, f"Input values dimension {dim} ({s_x}) doesn't match spec. ({s_spec})"
        x = self.proj(x)  # Project each patch into embedding, by convolution
        x = x.flatten(start_dim=2).transpose(1, 2)  # (B, D, H, W) -> (B, HW, D) for 2D patches
        x = self.norm(x)
...
class BoundedPatchEmbed(STPatchEmbed):
    def _define_position_embedding(
        ...
        self.patch_grid_coords = torch.nn.ParameterList(
            [torch.nn.Parameter(torch.arange(0.5, grid_s) / grid_s, requires_grad=False) for grid_s in self.grid_shape]
        )
        omegas.extend(
            [
                torch.nn.Parameter(calc_embed_omega(dim, period=x_max - x_min), requires_grad=False)
