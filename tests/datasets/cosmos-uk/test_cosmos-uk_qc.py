import pytest
import FRAME_FM.datasets.cosmosuk_dataset
...
def test_qc_mask_all_flags():
    ds = csv_loader.csv_to_xarray("FRAME-FM/tests/datasets/cosmos-uk/", 0b11111111111, [])
...
def test_qc_mask_missing():
    ds = csv_loader.csv_to_xarray("FRAME-FM/tests/datasets/cosmos-uk/", 0b0000000001, [])
...
def test_qc_mask_no_flags():
    ds = csv_loader.csv_to_xarray("FRAME-FM/tests/datasets/cosmos-uk/", 0b0000000000, [])
...
def test_flags_invalid():
    ...
    ds = csv_loader.csv_to_xarray("FRAME-FM/tests/datasets/cosmos-uk/", 0b0000000000, ["A"])
    assert np.array_equal(ds['LWIN'].values,[339.4, 329.4, 316.6, 316.5, 313.0, 316.8, 323.6, 336.2, 324.0],equal_nan=True)
