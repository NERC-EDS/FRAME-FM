...
def test_chessmet_dataset_with_transforms():
    ...
    assert get_main_vars(dataset.data) == ["precip"], (
        f"Expected dataset to have only 'precip' variable after subset transform, but got {dataset.data.data_vars}"
    )
...
def test_chessmet_dataset_retains_2d_coordinate_variables():
    ...
    assert "lat" in ds and "lon" in ds, (
        "Expected 'lat' and 'lon' to be ancinllary 2d coordinate variables in the dataset, but they were not found in the coordinates"
    )
    assert ds["lat"].ndim == 2 and ds["lon"].ndim == 2, (
        f"Expected 'lat' and 'lon' to be 2D coordinate variables, but got dimensions {ds['lat'].dims} and {ds['lon'].dims} respectively"
    )
    assert list(ds["lat"].coords.keys()) == ["x", "y"] and list(ds["lon"].coords.keys()) == ["x", "y"], (
        f"Expected 'lat' and 'lon' to have dimensions ['x', 'y'], but got: {ds['lat'].coords.keys()} and {ds['lon'].coords.keys()}"
    )
    ...
    assert not np.isnan(sample.numpy()).all(), (
        "Expected subsetted dataset to contain some valid data points, but all values were NaN - this may indicate an issue with the spatial subset transform resulting in an empty dataset"
    )
...
def test_land_cover_map_dataset_structure():
    ...
    assert "band_data" in ds.data_vars, (
        "Expected dataset to have 'band_data' variable after preprocessing, but it was not found in the data variables"
    )
    assert ds["band_data"].ndim == 3, (
        f"Expected 'band_data' to be a 3D variable, but got dimensions {ds['band_data'].dims}"
    )
...
def test_land_cover_map_with_transforms():
    ...
    assert ds["band_data"].shape == (10, 1300, 700), (
        f"Expected original 'band_data' shape to be (1, 1300, 700), but got {ds['band_data'].shape}"
    )

    sample = dataset[0]
    assert isinstance(sample, torch.Tensor), (
        f"Expected sample to be a torch.Tensor after applying transforms, but got {type(sample)}"
    )
    assert sample.ndim >= 2, (
        f"Expected sample to have at least 2 dimensions after applying transforms, but got {sample.ndim}"
    )
    assert sample.shape == (1, 10, 10), (
        f"Expected sample shape to be (1, 1500, 4500) after applying subset transform, but got {sample.shape}"
    )
...
def test_cosmosuk_dataset():
    ...
    assert type(data) is list, (
        f"Expected dataset.data to be a list of xarray Datasets (one per site), but got {type(data)}"
    )
    ...
    sample = dataset[0]
    assert len(sample) == 8
    assert set(sample.data_vars.keys()) == {
        "TDT1_VWC",
        "TDT2_VWC",
        "TDT3_VWC",
        "TDT4_VWC",
        ...
        "TDT7_VWC",
        "TDT8_VWC",
    }, f"Expected sample to contain the 8 TDT_VWC variables, but got {sample.data_vars.keys()}"
    assert "time" in sample.coords, "Expected sample to have 'time' coordinate, but it was not found in the coordinates"
    assert sample.TDT1_VWC.max().item() == 323.6, (
        f"Expected max value of TDT1_VWC to be 323.6 after applying QC mask, but got {sample.TDT1_VWC.max().item()}"
    )
