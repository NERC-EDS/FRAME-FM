...
def test_base_dataset():
    ...
    assert torch.isclose(sample.min(), torch.tensor(original_data.min()), atol=1e-5), (
        f"Sample min value {sample.min()} does not match original data min {original_data.min()}"
    )
    assert torch.isclose(sample.max(), torch.tensor(original_data.max()), atol=1e-5), (
        f"Sample max value {sample.max()} does not match original data max {original_data.max()}"
    )
...
def test_dataset_caching_basic():
    ...
    assert preprocessor_hash_key in cached_ds.attrs, (
        f"Cached dataset should have the hash attribute {preprocessor_hash_key}"
    )
    ...
    assert preprocessor_hash_key not in dataset_no_cache.data.attrs, (
        f"Non-cached dataset should not have the hash attribute {preprocessor_hash_key}"
    )
