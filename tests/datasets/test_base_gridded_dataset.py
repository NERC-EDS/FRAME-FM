...
from FRAME_FM.utils.data_utils import hash_preprocessors, safely_remove_dir, preprocessor_hash_key
...
def test_base_gridded_dataset():
    ...
    assert isinstance(sample, torch.Tensor)
    assert sample.shape == (1, 130, 70), (
        f"Expected sample shape to be (1, 130, 70) after resampling and squeezing, but got {sample.shape}"
    )
...
def test_base_timeseries_dataset_with_transforms():
    ...
    assert len(dataset) > 0, (
        f"Expected dataset length to be greater than 0 after applying time range and stride, but got {len(dataset)}"
    )
    sample = dataset[0]
    next_sample = next(iter(dataset))
    assert isinstance(sample, torch.Tensor), (
        f"Expected sample to be a torch.Tensor after transforms, but got {type(sample)}"
    )
    assert isinstance(next_sample, torch.Tensor), (
        f"Expected next_sample to be a torch.Tensor after transforms, but got {type(next_sample)}"
    )


def test_base_timeseries_dataset_nc_file():
    dataset = BaseGriddedTimeSeriesDataset(data_uri=NC_URI, time_stride=8)
    sample = dataset[0]
...
def test_base_timeseries_dataset_with_cache():
    ...
    dataset = BaseGriddedTimeSeriesDataset(
        data_uri=TIMESERIES_URI, preprocessors=preprocessors, time_stride=8, cache_dir=cache_dir, chunks="auto"
    )
    ...
    dataset_no_cache = BaseGriddedTimeSeriesDataset(
        data_uri=TIMESERIES_URI, preprocessors=preprocessors, time_stride=8, cache_dir=None, chunks="auto"
    )

    # Compare the datasets
    assert dataset.data.equals(dataset_no_cache.data), (
        "Expected cached dataset to have the same data as non-cached dataset after preprocessing, but they differ"
    )
    assert (dataset.data["d2m"].values == dataset_no_cache.data["d2m"].values).all(), (
        "Expected cached dataset variable values to match non-cached dataset variable values after preprocessing, but they differ"
    )
    ...
    assert cached_hash is not None, (
        f"Expected cached dataset to have the hash attribute {preprocessor_hash_key}, but it was not found"
    )
    ...
    assert non_cached_hash is None, (
        f"Expected non-cached dataset to not have the hash attribute {preprocessor_hash_key}, but it was found with value {non_cached_hash}"
    )
...
def test_base_timeseries_dataset_caching_backends(caching_backend):
    ...
    subset = {
        "type": "subset",
        "time": ("2000-01-01 00:00:00", "2000-01-31 23:00:00"),
        "latitude": (60, -30),
        "longitude": (40, 100),
    }
    ...
    dataset = BaseGriddedTimeSeriesDataset(
        data_uri=data_uri, preprocessors=[subset.copy()], time_stride=8, cache_dir=cache_dir, chunks=chunks
    )
    ...
    # Create a version without cache and compare the results - initially the dataset after construction
    dataset_no_cache = BaseGriddedTimeSeriesDataset(
        data_uri=data_uri, preprocessors=[subset.copy()], time_stride=8, cache_dir=None, chunks=chunks
    )
    ...
    )  # Select a small subset of the data for comparison to speed up the process
    ds2 = dataset_no_cache.data.isel(
        time=0, latitude=0, longitude=slice(-3, None)
    )  # Select the same small subset of the data for comparison
    ...
    assert (ds1["d2m"].values == ds2["d2m"].values).all(), (
        "Expected cached dataset variable values to match non-cached dataset variable values after preprocessing, but they differ"
    )
    ...
    assert Path(croissant_path).exists(), (
        f"Expected croissant file {croissant_path} to be created for cached dataset, but it was not found"
    )
    # Load croissant file and check it has a key called "@context"
    with open(croissant_path, "r") as f:
        croissant_data = json.load(f)
    assert "@context" in croissant_data, (
        f"Expected croissant file {croissant_path} to contain a key called '@context', but it was not found"
    )
