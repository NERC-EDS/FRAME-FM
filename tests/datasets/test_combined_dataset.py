...
def test_zip_dataset():
    ...
    for index in range(0, len(zip_dataset), len(zip_dataset) // 4):
        ...
        assert len(sample) == 2, f"ZipDataset failure: # of items #{index} not number of datasets ({len(sample)} != 2)."
        assert_close(
            sample[0],
            pre_dataset[index],
            equal_nan=True,
...
def test_corresponding_tiles_dataset():
    ...
    for index in range(0, len(corresponding_dataset), len(corresponding_dataset) // 4):
        ...
        assert len(sample) == 2, (
            f"CorrespondingTilesDataset failure: # of items not # of datasets ({len(sample)} != 2)."
        )
        ...
        assert_close(
            ...
            pre_sample_vals,
            equal_nan=True,
            msg=(f"CorrespondingTilesDataset failure: first of item #{index} values not #{index} of first dataset."),
        )
        pre_xy_c = first_sample_locs[:2].mean(dim=[1, 2, 3])
        if (
            (pre_xy_c[0].numpy() < geotiff_dataset.data["x"].min())
