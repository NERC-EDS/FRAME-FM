...
Hydra configuration files defining models, data modules, trainers, logging,
...
The top-level ``config.yml`` defines the default configuration by including
entries from the subfolders:
...
These files are merged together into one final config object, which is passed
...
- ``model/``
  ...
  Each file here must contain a ``_target_`` field that points to a Python
...
- ``data/``
  ...
  Each file should describe how training/validation/test data is created or
  loaded.

- ``trainer/``

  Contains configurations for PyTorch Lightning’s Trainer: number of epochs,
  devices, precision settings, logging frequency, etc.
